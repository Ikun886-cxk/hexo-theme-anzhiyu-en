const anzhiyu = {
  ...
  downloadImage: function (imgsrc, name) {
    ...
    if (rm.downloadimging == false) {
      ...
      anzhiyu.snackbarShow("Downloading now, please wait", false, 10000);
      setTimeout(function () {
        let image = new Image();
        // 解决跨域 Canvas 污染问题
        ...
        image.src = imgsrc;
        anzhiyu.snackbarShow("Images are blind watermarked, please observe the copyright agreement");
  ...
  switchCommentBarrage: function () {
    ...
    if (commentBarrage) {
      if (window.getComputedStyle(commentBarrage).display === "block") {
        ...
        anzhiyu.snackbarShow("✨ Closed comment pop-ups");
        document.querySelector(".menu-commentBarrage-text").textContent = "Show hot reviews";
        document.querySelector("#consoleCommentBarrage").classList.remove("on");
        localStorage.setItem("commentBarrageSwitch", "false");
      } else {
        commentBarrage.style.display = "block";
        document.querySelector(".menu-commentBarrage-text").textContent = "Close Hot Comments";
        document.querySelector("#consoleCommentBarrage").classList.add("on");
        anzhiyu.snackbarShow("✨ Comment pop-ups have been opened");
  ...
  addFriendLink() {
    ...
    input.value =
      "Nickname (please do not include words such as blog)：\nWebsite address (blog address required, please do not submit personal homepage)：\navatar picture url (please provide as clear a picture as possible, I will upload to my own image bed)：\nDescription：\nSite Screenshot (optional)：\n";
  ...
  travelling() {
    ...
    fetch(fetchUrl)
      ...
      .then(json => {
        ...
        Snackbar.show({
          text:
            "Click the go button to enter a random link, the security and availability of the jump site is not guaranteed. This time, the random link to this site is a friend link：「" + name + "」",
          duration: 8000,
          pos: "top-center",
          actionText: "Go to",
  ...
  musicToggle: function (changePaly = true) {
    ...
    let msgPlay = '<i class="anzhiyufont anzhiyu-icon-play"></i><span>Play Music</span>'; // 此處可以更改為你想要顯示的文字
    let msgPause = '<i class="anzhiyufont anzhiyu-icon-pause"></i><span>Pause music</span>'; // 同上，但兩處均不建議更改
    if (anzhiyu_musicPlaying) {
      navMusicEl.classList.remove("playing");
      document.getElementById("menu-music-toggle").innerHTML = msgPlay;
      document.getElementById("nav-music-hoverTips").innerHTML = "Music is paused";
  ...
  playMusic(songs) {
    ...
    console.info("Randomized songs：", selectRandomSong, "This random song：", randomSong.name);
  },
  // 音乐节目切换背景
  ...
  totraveling: function () {
    anzhiyu.snackbarShow("This is a redirect to the blog of a member of the 开往 project. The security and availability of the redirected site is not guaranteed.", !1, 5000);
    setTimeout(function () {
